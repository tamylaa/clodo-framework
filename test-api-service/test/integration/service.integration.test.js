...
describe('Test Api Service - Integration Tests', () => {
  let baseUrl;
  let originalFetch;

  beforeAll(() => {
    baseUrl = process.env.TEST_URL || 'https://test-api-service-dev.test.example.com';
    ...
    if (!process.env.TEST_URL) {
      ...
      global.fetch = async (input, init) => {
        ...
        const serviceName = 'test-api-service';
  ...
  describe('Health Check Endpoint', () => {
    ...
    test('returns valid health check response', async () => {
      ...
      expect(json.service).toBe('test-api-service');
  ...
  describe('API Endpoints', () => {
    test('base API path is accessible', async () => {
      const response = await fetch(`${baseUrl}/api/v1/test/api-service`);
  ...
  describe('Middleware Integration', () => {
    test('request headers added', async () => {
      ...
      expect(response.headers.get('x-service')).toBe('test-api-service');
