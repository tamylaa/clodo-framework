...
function generateSecrets(options = {}) {
  ...
  console.log('================================');
  console.log('');
  console.log('⚠️  IMPORTANT: Store these secrets securely and never commit them to git!');
  console.log('');
  ...
  if (secretsFile && existsSync(secretsFile)) {
    try {
      ...
      console.log(`📂 Reusing existing secrets for domain '${domainName}' (consistent across services)`);
      console.log(`🕒 Originally generated: ${existing.generated}`);
      console.log('');
      console.log('🔄 IMPORTANT: Using existing secrets ensures all services can authenticate with each other');
    } catch (error) {
      console.log('⚠️  Could not load existing secrets, generating new ones...');
    }
  } else if (domainName) {
    console.log(`🆕 Creating new secrets for domain '${domainName}'`);
  }
  ...
  if ((saveToFile || domainName) && secretsFile) {
    try {
      ...
      console.log('📋 Other services can load these secrets for consistent authentication');
    } catch (error) {
      console.log(`⚠️  Could not save secrets to file: ${error.message}`);
    }
  }
  ...
  console.log('');
  console.log('🚀 To set these in Cloudflare Workers:');
  console.log('');
  ...
  console.log('');
  console.log('💡 You can also set these individually when prompted by the deployment script.');
