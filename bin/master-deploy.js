...
// Enterprise module imports - powered by our new shared modules
import { askUser, askYesNo, askChoice, closePrompts, ConfigurationCacheManager, EnhancedSecretManager } from '@tamyla/lego-framework/utils/deployment';
...
`;
  ...
  async showEnterpriseSuccessSummary() {
    ...
    console.log(`   📝 Audit Log: audit-logs/deployments/session-${this.state.deploymentId}.log`);
    console.log(`   📊 Report: audit-reports/deployment-${this.state.deploymentId}.html`);
    console.log(`   🔐 Secrets: secrets/distribution/${this.config.domain}/`);
    console.log(`   ⚙️  Config: config-cache/domains/${this.config.domain}/`);
