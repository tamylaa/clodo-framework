# Clodo Framework - Setup Helper Functions
...
function Write-SetupLog {
    ...
    if ($env:CLODO_SETUP_LOG) {
        $logMessage | Out-File -FilePath $env:CLODO_SETUP_LOG -Append -Encoding UTF8
    }
}
...
function New-ServiceFromTemplate {
    ...
    # Scan templates for suspicious test/example strings and warn maintainers (prevents accidental test data shipping)
    $suspiciousPatterns = @('test-service','test\.example\.com','testcorp','test-worker','SERVICE_DOMAIN = "test','secrets','-secrets.json')
    $found = @()
    Get-ChildItem -Path $templateDir -Recurse -File | ForEach-Object {
        foreach ($pat in $suspiciousPatterns) {
            try {
                if (Select-String -Path $_.FullName -Pattern $pat -SimpleMatch -Quiet) {
                    $found += "{0}:{1}" -f $_.FullName, $pat
                }
            } catch { }
        }
    }
    if ($found.Count -gt 0) {
        Write-SetupLog "WARNING: Template directory contains suspicious test data patterns. Files: $($found -join '; ')" -Level "WARN" -Component "Template"
    }

    # Copy template files (excluding config and secrets directories to avoid copying test/example configs and stored secrets)
    # Config and secrets should be generated by framework logic or managed securely, not copied verbatim from templates.
    Get-ChildItem -Path $templateDir -Recurse -File | Where-Object { $_.FullName -notmatch '\\(config|secrets)\\' } | ForEach-Object {
        $relative = $_.FullName.Substring($templateDir.Length).TrimStart('\')
        $dest = Join-Path $serviceDir $relative
...
// Generated by Clodo Framework Setup
...
function Initialize-ServiceGit {
    ...
    param(
        ...
        [Parameter(Mandatory=$false)]
        [string]$InitialCommitMessage = "Initial commit - Clodo Framework service"
    )
...
$(if ($ServiceInfo.Description) { $ServiceInfo.Description } else { "A Clodo Framework $ServiceType service" })

## Overview

This service was generated using the Clodo Framework and follows established patterns for:
...
$(if ($ServiceInfo.Features) {
    ...
    "- Standard Clodo Framework features"
})
...
This service uses the Clodo Framework for:
