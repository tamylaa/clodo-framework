...
`;
  ...
  generateDomainsConfig(serviceName, config, domainInfo) {
    let domainsConfig = `import { createDomainConfigSchema } from '@tamyla/lego-framework';
...
`;
  ...
  generatePackageJson(serviceName, config) {
    const packageJson = {
      name: serviceName,
      version: '1.0.0',
      description: `${config.type} service created with Lego Framework`,
      type: 'module',
      main: 'dist/index.js',
      scripts: {
      ...
      keywords: ['cloudflare', 'workers', 'lego-framework', config.type],
...
${config.type} service created with Lego Framework.
...
`;
  ...
  createServiceStructure(serviceName, config, configs) {
    ...
    const workerContent = `import { LegoWorker } from '@tamyla/lego-framework/worker';
import { domains } from './config/domains.js';

export default {
  async fetch(request, env, ctx) {
    const worker = new LegoWorker({
      domains,
      environment: env
    });
