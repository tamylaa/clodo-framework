# Clodo Framework CLI Tools

This directory contains executable scripts and command-line tools for the Clodo Framework.

## Overview

The CLI tools provide a conversational interface for:
- Creating new Clodo services
- Deploying services to Cloudflare
- Validating service configurations
- Managing service lifecycles

## For Framework Users

If you're using `@tamyla/clodo-framework` in your project, use the public API exported via package.json:
...
import { healthCheckWithBackoff } from '@tamyla/clodo-framework/dist/cli/shared/monitoring/health-checker.js';
```

## CLI Categories

### Core Commands
- `clodo-service create` - Create services with conversational setup
- `clodo-service deploy` - Deploy services with smart credential handling
...
### Security
Security management and authentication tools:
- `security-cli.js` - Security operations CLI

## Architecture Notes

This directory contains CLI-specific implementations that are distributed with the package for end-user execution. The shared utilities in `../lib/` provide common functionality used by both CLI tools and the library API.
