...
export class InteractiveConfirmation {
  ...
  static displayDeploymentSummary(config, deploymentState) {
    ...
    console.log(`🔗 URL: ${config.worker?.url || 'Not configured'}`);
    
    // Database info from resources
    const database = deploymentState.resources?.database;
    if (database) {
      console.log(`🗄️ Database: ${database.name} (${database.id})`);
    } else {
    ...
    if (secrets) {
      ...
      console.log(`🔑 Secrets: ${secretCount} configured`);
    } else {
  ...
  static displayEnterpriseDeploymentSummary(config, deploymentState) {
    ...
    console.log(`⚡ Worker: ${config.worker.name}`);
    console.log(`🔗 URL: ${config.worker.url}`);
    console.log(`🗄️ Database: ${config.database?.name || 'None'} (${config.database?.id || 'No ID'})`);
    console.log(`🔑 Secrets: ${config.secrets?.keys ? Object.keys(config.secrets.keys).length : 0} configured`);
    console.log(`🆔 Deployment ID: ${deploymentState.deploymentId}`);
    console.log(`🔍 Validation: ${config.deployment?.validationLevel || 'standard'}`);
    console.log(`📋 Audit: ${config.deployment?.auditLevel || 'basic'}`);
  }
