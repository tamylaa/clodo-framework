...
export class InteractiveDomainInfoGatherer {
  ...
  async configureWorker(config) {
    ...
    if (!confirmWorkerConfig) {
      let customWorkerName = await askUser(
      ...
      if (customWorkerName.includes('.') || customWorkerName.includes('/') || customWorkerName.includes('://')) {
        console.log('   ⚠️  Worker name should be just the name, not a full URL');
        console.log('   💡 Example: "my-service" not "my-service.workers.dev"');
        customWorkerName = customWorkerName.split('.')[0]; // Extract just the first part
        console.log(`   🔧 Using extracted name: ${customWorkerName}`);
      }
