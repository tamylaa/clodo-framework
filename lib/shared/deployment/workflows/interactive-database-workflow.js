...
export class InteractiveDatabaseWorkflow {
  ...
  async checkExistingDatabase(name, credentials = {}) {
    ...
    try {
      ...
      if (exists) {
        ...
        if (credentials.apiToken && credentials.accountId) {
          try {
            ...
            console.log(`   ⚠️  Could not get database ID from API: ${apiError.message}`);
        ...
        try {
          ...
          console.log(`   ⚠️  Could not extract database ID: ${cliError.message}`);
        }
      ...
      console.log(`   ⚠️  Error checking database existence: ${error.message}`);
      console.log('   ℹ️  Assuming database does not exist');
      return { exists: false };
    }
  }
