...
export class InteractiveDeploymentCoordinator {
  ...
  async executeDeployment() {
    ...
    const result = await Clodo.deploy({
      ...
      workerName: this.deploymentState.config.worker?.name, // Pass the collected worker name
      databaseName: this.deploymentState.resources.database?.name, // Pass the collected database name
