...
export async function authenticate() {
  try {
    ...
    await execAsync('npx wrangler login', { stdio: 'inherit' });
...
export async function listWorkers(options = {}) {
  ...
  if (apiToken && accountId) {
    const { CloudflareAPI } = await import('@tamyla/clodo-framework/utils/cloudflare');
  ...
  try {
    if (apiToken) {
      const command = process.platform === 'win32'
        ? `powershell -Command "$env:CLOUDFLARE_API_TOKEN = '${apiToken}'; npx wrangler list"`
        : `CLOUDFLARE_API_TOKEN=${apiToken} npx wrangler list`;
      const { stdout: list } = await executeWithRateLimit(command, 'workers');
      return list;
    } else {
...
export async function deploySecret(key, value, env = 'production', options = {}) {
  const { apiToken, accountId, scriptName } = options;
  
  // Use API-based operation if credentials provided
  if (apiToken && accountId && scriptName) {
    const { CloudflareAPI } = await import('@tamyla/clodo-framework/utils/cloudflare');
    const cf = new CloudflareAPI(apiToken);
    await cf.putWorkerSecret(accountId, scriptName, key, { text: value });
    return;
  }
  ...
  if (apiToken) {
    const command = process.platform === 'win32'
      ? `powershell -Command "$env:CLOUDFLARE_API_TOKEN = '${apiToken}'; Write-Output '${value}' | npx wrangler secret put ${key} --env ${env}"`
      : `CLOUDFLARE_API_TOKEN=${apiToken} echo "${value}" | npx wrangler secret put ${key} --env ${env}`;
...
export async function deleteSecret(key, env = 'production', options = {}) {
  ...
  if (apiToken) {
    const command = process.platform === 'win32'
      ? `powershell -Command "$env:CLOUDFLARE_API_TOKEN = '${apiToken}'; npx wrangler secret delete ${key} --env ${env}"`
      : `CLOUDFLARE_API_TOKEN=${apiToken} npx wrangler secret delete ${key} --env ${env}`;
...
export async function listSecrets(env = 'production', options = {}) {
  ...
  if (apiToken) {
    const command = process.platform === 'win32'
      ? `powershell -Command "$env:CLOUDFLARE_API_TOKEN = '${apiToken}'; npx wrangler secret list --env ${env}"`
      : `CLOUDFLARE_API_TOKEN=${apiToken} npx wrangler secret list --env ${env}`;
    const { stdout: list } = await executeWithRateLimit(command, 'workers');
    return list;
  } else {
...
export async function createDatabase(name, options = {}) {
  ...
  try {
    if (apiToken) {
      const command = process.platform === 'win32'
        ? `powershell -Command "$env:CLOUDFLARE_API_TOKEN = '${apiToken}'; npx wrangler d1 create ${name}"`
        : `CLOUDFLARE_API_TOKEN=${apiToken} npx wrangler d1 create ${name}`;
      const { stdout: output } = await executeWithRateLimit(command, 'd1');
...
export async function deleteDatabase(name, options = {}) {
  ...
  if (apiToken) {
    ...
    const command = process.platform === 'win32'
      ? `powershell -Command "$env:CLOUDFLARE_API_TOKEN = '${apiToken}'; npx wrangler d1 delete ${name} --skip-confirmation"`
      : `CLOUDFLARE_API_TOKEN=${apiToken} npx wrangler d1 delete ${name} --skip-confirmation`;
...
export async function runMigrations(databaseName, env = 'production', options = {}) {
  ...
  try {
    ...
    const result = await errorRecovery.executeWithRecovery(async () => {
      if (apiToken) {
        const command = process.platform === 'win32'
          ? `powershell -Command "$env:CLOUDFLARE_API_TOKEN = '${apiToken}'; npx wrangler d1 migrations apply ${databaseName} --env ${env} --remote"`
          : `CLOUDFLARE_API_TOKEN=${apiToken} npx wrangler d1 migrations apply ${databaseName} --env ${env} --remote`;
        await executeWithRateLimit(command, 'd1');
      } else {
...
export async function validatePrerequisites(options = {}) {
  const { checkAuth = false, checkNetwork = false, verbose = false } = options;
    ...
    checks.push({
      ...
      checkFunction: async () => {
        try {
          ...
          return { status: 'ok', version: 'Connected' };
        } catch (error) {
          if (error.name === 'AbortError') {
            return { status: 'failed', error: 'Connection timeout' };
          }
          return { status: 'ok', version: 'Connected (API responded)' };
        }
      }
    });
  }
  ...
  for (const check of checks) {
    try {
      ...
      if (check.async && check.checkFunction) {
        ...
        const { stdout: version } = await execAsync(check.command, {
          encoding: 'utf8',
          timeout: 10000
        });
        result = { status: 'ok', version: version.trim() };
        ...
        result = { status: 'skipped', version: 'N/A' };
      }
