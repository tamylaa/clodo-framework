/**
 * Test Integration Service - Service Handlers
 *
...
export function createServiceHandlers(serviceConfig, env) {
  return {
    async handleRequest(request, ctx) {
      ...
      // API routes
      if (path.startsWith('/api/v1/test/integration-service')) {
      ...
      return new Response(JSON.stringify({
        ...
        availableEndpoints: [
          '/health',
          '/api/v1/test/integration-service/*'
        ]
      }), {
        status: 404,
    ...
    async handleApiRequest(request, ctx, serviceConfig, env) {
      try {
        const url = new URL(request.url);
        const path = url.pathname.replace('/api/v1/test/integration-service', '');
